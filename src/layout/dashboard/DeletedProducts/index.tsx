...
const [totalUnavailableNewProducts, setTotalUnavailableNewProducts] = useState<number>(0);
  ...
  const handleSearch = useCallback(
    (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      setSearch(value);
      if (value === '') {
        setProducts(initialProducts); // Remettre les produits initiaux lorsque la recherche est vide
      } else {
        const filteredProducts = initialProducts.filter(
          (product: newProduct) =>
            ...
            (product.supprime || !product.supprime) // Inclure également les produits supprimés
        );
        setProducts(filteredProducts); // Mettre à jour les produits filtrés
      }
    },
    [initialProducts]
  );
